...
int[,] serialMulRes = new int[N, N]; //результат выполнения умножения матриц в однопотоке, последовательно
...
SerialMatrixMul(firstMatrix, secondMatrix); // последовательное умножение
...
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы");
    int eachThreadCalc = N / THREADS_NUMBER; // сколько в каждом потоке
    Thread[] arr = new Thread[2];
    var threadsList = new List<Thread>(); // здесь сохраняем наши потоки
...
bool EqualityMatrix(int[,] fmatrix, int[,] smatrix) // сравнивает 2 матрицы
{
    bool res = true;
